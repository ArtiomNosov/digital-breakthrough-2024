...
with open('technology_keywords.pkl', 'rb') as f:
    technology_keywords = pickle.load(f)
...
def extract_keywords(content):
    tokens = word_tokenize(''.join(content).lower())
...
def extract_experience(content):
    match = re.search(r'Требуемый опыт:\s*(.*?)[,.]', content)
    if match:
